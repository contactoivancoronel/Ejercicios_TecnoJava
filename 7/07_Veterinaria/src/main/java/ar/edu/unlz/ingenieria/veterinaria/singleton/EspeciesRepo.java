...
public class EspeciesRepo {
	...
	public Especie get(String nombre) {
		Especie especieBuscada = this.listadoEspecies.stream().filter((e) -> e.getNombre().equals(nombre)).findFirst()
				.orElse(null);
	...
	public Especie getById(int id) {
		return this.stream().filter(e -> e.getId() == id).findFirst().orElse(null);
	}
